...
from pytorch_pretrained_bert.modeling import BertForQuestionAnswering,BertForSpan
...
class SquadExample(object):
    ...
    def __repr__(self):
        ...
        if self.start_position:
            s += ", start_position:vocab.txt %d" % (self.start_position)
...
def make_qid_to_has_ans(dataset):
    ...
    for article in dataset:
        ...
        for qa in article['qas']:
            qid_to_has_ans[qa['qid']] = bool(qa['answer'])
...
def get_raw_scores(dataset, preds):
    ...
    for article in dataset:
        ...
        for qa in article['qas']:
            qid = qa['qid']
            gold_answers = normalize_answer(qa['answer'])
            if not gold_answers:
                gold_answers = ''
            if qid not in preds:
...
def main(args):
    ...
    if args.do_train:
        ...
        model = BertForSpan.from_pretrained(
            args.model, cache_dir=PYTORCH_PRETRAINED_BERT_CACHE)
        # model(arc_hidden=256, arc_depth=1, arc_activation='ReLU',arc_dropout=.33)
        # if args.fp16:
        #     model.half()
        model.to(device)
        if n_gpu > 1:
    ...
    if args.do_test:
        ...
        model = BertForSpan.from_pretrained(args.output_dir)
        if args.fp16:
            model.half()
        model.to(device)
...
if __name__ == "__main__":
        ...
        parser.add_argument("--train_file", default=None, type=str,
                            help="span json for training. E.g., train.json")
        parser.add_argument("--dev_file", default=None, type=str,
                            help="SQuAD json for predictions. E.g., dev.json or test.json")
